...
for snr_idx = 1:length(snr)
    ...
    %Modulate
    paradata = reshape(coded_bits,length(coded_bits)/ml,ml);
    ...
    cha_amp_mtx = repmat(abs(H).^2,1,NumSyms);      %不同
    freq_data_cha_equ = freq_data.*conj(cha_mtx)./cha_amp_mtx;    %不同
    ...
    DecodedBits = vitdec(SerialBits(1:length(coded_bits)),trellis,tb,'trunc','hard');
