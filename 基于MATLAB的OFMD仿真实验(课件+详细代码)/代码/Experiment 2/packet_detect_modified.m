...
delay_xcorr = rx_signal(1:search_win+2*D).* ...
            conj(rx_signal(1*D+1:search_win+3*D));  %732*1
...
%改成for循环的写法：                                   2019/05/18
ma_delay_xcorr_ss = zeros(700,1);
for i = 1:700
    recorder = 0;
    for j = i : (i+2*D- 1)      % 1-32、2-33、3-34...700-731
        recorder = recorder + (delay_xcorr(j+1));             % 从第33个开始算所以+1   ******
    end
    ma_delay_xcorr_ss(i) = abs(recorder);
end
%-------------------------------------------------------------------------
...
%改成for循环的写法：                                 2019/05/18
ma_rx_pwr_s = abs(rx_signal(1*D+1 : search_win+3*D)).^2 ;   %17-748
ma_rx_pwr_ss = zeros(700,1);
for i = 1:700
    recorder = 0;
    for j = i : (i+2*D-1)                           % 2-33、3-34...700-731、701-732
        recorder = recorder + ma_rx_pwr_s(j+1);     % 从最最初第33个开始算所以+1   ******
    end
    ma_rx_pwr_ss(i) = recorder;
end
%-------------------------------------------------------------------------


% The decision variable                       
delay_len = length(ma_delay_xcorr_ss);    % 700*1
ma_M = ma_delay_xcorr_ss(1:delay_len)./ma_rx_pwr_ss(1:delay_len);    % 一次性计算所有相邻符号相关性mn=|Cn|/Pn
plot(ma_M);
